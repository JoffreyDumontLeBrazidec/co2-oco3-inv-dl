...
# download
df_pos = pd.read_csv (dir_save_current_model + "/" + "list_infos_predictions.csv", index_col=0)
...
df_pos["tt_plume_var_quantile_1e2"]     = pd.qcut(df_pos["tt_test_plume_var"]*100, q=4, precision=1)
df_pos["ratio_var_quantile_1e2"]        = pd.qcut(df_pos["ratio_var"]*100, q=4, precision=2)
df_pos["tt_plume_mean_quantile_1e2"]    = pd.qcut(df_pos["tt_test_plume_mean"]*100, q=4, precision=2)
df_pos["tt_test_image_cover_quantile"]  = pd.qcut(df_pos["tt_test_image_cover"], q=4, precision=2)
df_pos["hour_quantile"]                 = pd.qcut(df_pos["hour"], q=4, precision=2)

df_pos.to_csv (dir_save_current_model + "/" + "list_infos_pos_predictions.csv")

# prepare plot
variable_axs    = ["tt_plume_mean_quantile_1e2", "tt_plume_var_quantile_1e2", "pred_success", "tt_test_image_cover_quantile"]
...
for index_ax, ax in zip (range(len(axs)), axs):
    sns.histplot            (data=df_pos, x=output, hue=variable_axs[index_ax], ax=ax, log_scale=False, element="step", cumulative=False, stat="density", common_norm=False)
    #ax.set_ylim             (0.0,1)
