...
title = "val_accuracy_violins.pdf"
...
xlabel_axs      = ["Time (input)", "Wind info. (input)", "Dynamic info. (input)", "Scale (input)", "dataset", "N_epochs_quantile"]
text_axs        = ["(a)", "(b)", "(c)", "(d)", "(e)", "(f)"]
ylabel_axs      = ["test accuracy (best)", "", "test accuracy (best)", "", "test accuracy (best)", ""]
output          = "test_accuracy.best"
xlabels_ticks_size_axs  = [6,6,6,6,4,4]
...
savefig_name = os.path.join(dir_all_models, title)
plt.savefig (savefig_name, bbox_inches= 'tight')
print ("PLOT:", savefig_name)
