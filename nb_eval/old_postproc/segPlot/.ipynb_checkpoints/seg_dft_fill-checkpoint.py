...
df        = pd.read_pickle(path_to_dataset + "/" + "df_infos_dataset.plk")
...
tt_test_image_cover = (np.sum((tt_test>1e-5), axis=(1,2)) / (tt_test.shape[1]*tt_test.shape[2]))[:,0]
...
for index_test, norm_index_test in zip(df_test.index.values, range (len(df_test.index.values))):
...
df_test.to_csv (dir_save_current_model + "/" + "list_infos_predictions.csv")
