...
class Eval_Shuffler:
    ...
    def __init__(
        ...
        if shuffle_indices is None:
            self.shuffle_indices = np.random.permutation(N_data)
        else:
        ...
        self.ds_inds = {
            "train": list(self.shuffle_indices[0:N_train]),
            "valid": list(self.shuffle_indices[N_train : N_train + N_valid]),
            "test": list(self.shuffle_indices[N_train + N_valid :]),
        }
...
class Input:
    ...
    def __init__(
        ...
        self.scaler = scaler
        self.fill_channel0(ds, config)
        
        
        def fill_channel0(self, ds, config):
            """Fill channel 0 of field Input with xco2."""
            xco2 = np.expand_dims(ds.xco2.values, -1)
            print("mean, var xco2", np.mean(xco2), np.var(xco2))
            xco2 = self.add_noise(
                xco2,
            ...
            [f_train, f_valid, f_test] = self.standardise(f_train, f_valid, f_test)
        ...
        def fill_channel12
            """Fill channels 1 and 2 of field Input."""
            # Here you want a function to know what to put at channel 1, and 2
            # if regression, you want 3 fields so you either copy zero, one or two
            # if segmentation, you just want additional fields so you copy one then the other
            if con
            
            if config.get("data.output.label.choice") == "regression" and self.train.shape[-1] == 1:
    ...
    def standardise(self, f_train, f_valid, f_test):
        """Standardise data according to f_train or given scaler."""
        if self.scaler == None:
            self.scaler = preprocessing.StandardScaler()
            self.scaler.fit(f_train.reshape(-1, f_train.shape[-1]))

        f_train = self.scaler.transform(f_train.reshape(-1, f_train.shape[-1])).reshape(
            f_train.shape
        )
        f_valid = self.scaler.transform(f_valid.reshape(-1, f_valid.shape[-1])).reshape(
            f_valid.shape
        )
        f_test = self.scaler.transform(f_test.reshape(-1, f_test.shape[-1])).reshape(
            f_test.shape
        )        
...
class Output:
    ...
    def get_regression(self, ds, eval_shuffler, config):
        ...
        self.get_train_valid_test(emiss, eval_shuffler)
        
    def get_train_valid_test(self, data, eval_shuffler):
